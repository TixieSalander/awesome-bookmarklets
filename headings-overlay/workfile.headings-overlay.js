// This one was found online but I don't remember where, it's been on my browser for so many years 😬

(function() {
	function updateHeight() {
		container.style.height = "0px", container.style.height = doc.scrollingElement.scrollHeight + "px"
	}

	function getOutline() {
		for (var e = 0, t = document.querySelectorAll('h1,h2,h3,h4,h5,h6,h7,[role="heading"]'), i = [], o = 0; o < t.length; o++) {
			var n = t[o],
				l = isVisible(t[o]),
	...
	function outlineToHTML(e) {
		for (var t = "", i = 0; i < e.length; i++) {
			var o = e[i],
				n = o.el;
			t += '<li class="', t += o.wrong ? "wrong-level" : "correct-level", t += o.visible ? "" : " hidden", t += o.visuallyhidden ? " visuallyhidden" : "", t += '" style="margin-left: ' + o.level + 'em;">', t += '<a href="#' + i + '" target="_top">', t += '<span class="level" data-level="' + o.level + '"></span> ', t += '<span class="text">' + htmlEntities(n.textContent.replace(/\s+/g, " ")) + "</span>", t += "</a>", t += "</li>"
		}
	...
	function highlightElement(e, t) {
		t || (e.scrollIntoViewIfNeeded ? e.scrollIntoViewIfNeeded() : e.scrollIntoView && e.scrollIntoView()), setTimeout(function() {
			for (var t = e.getBoundingClientRect(), i = e.parentElement; !t.height && !t.width && !t.left && !t.top && i;) t = i.getBoundingClientRect(), !1, i = i.parentElement;
			i && ((t = {
				left: t.left,
				top: t.top,
				bottom: t.bottom,
				right: t.right
			}).left = Math.min(window.innerWidth, t.left), t.right = Math.max(0, t.right), t.top = Math.min(window.innerHeight, t.top), t.bottom = Math.max(0, t.bottom), document.getElementById(highlighterEl.id) || document.body.appendChild(highlighterEl), highlighterEl.style.left = t.left - 10 + "px", highlighterEl.style.width = t.right - t.left + 20 + "px", highlighterEl.style.top = t.top - 10 + "px", highlighterEl.style.height = t.bottom - t.top + 20 + "px", highlighterEl.style.display = "block")
		}, 100)
	}
	...
	function highlightLink(e) {
		for (var t = doc.querySelectorAll("#headings a"), i = t.length - 1; i >= 0; i--) t[i] === e ? t[i].classList.add("is-active") : t[i].classList.remove("is-active")
	}

	function handleElementHover(e) {
		for (var t = e.target, i = document.body.querySelectorAll("*"), o = !1, n = i.length - 1; n >= 0; n--) {
			var l = i[n];
			if (o) {
				for (var r = outline.length - 1; r >= 0; r--)
					if (outline[r].el === l && outline[r].visible) return highlightElement(outline[r].el, !0), void highlightLink(doc.querySelector('#headings a[href="#' + r + '"]'))
			} else l === t && (o = !0, n++)
		}
		highlightLink(null)
	}
	...
	var containerId = "a11y-bookmarklet",
		containerStyle = "position: fixed; top: 0; right: 0; max-height: 100%; box-shadow: 0 0 80px rgba(0,0,0,0.3); width: 20%; min-width: 320px; max-width: 450px; z-index: 1000001;",
		highlighterEl = document.createElement("DIV");
	highlighterEl.id = "h1-a11y-highlighterelement", highlighterEl.style.cssText = "pointer-events: none; position: fixed; border: 1px dashed #0081BE; box-shadow: 0 0 54px 0 rgba(0,84,150,0.3); display: none; z-index: 1000000; transition: all 200ms;";
	var container = document.getElementById(containerId);
	container && document.body.removeChild(container), (container = document.createElement("DIV")).id = containerId, container.style.cssText = containerStyle;
	var iframe = document.createElement("IFRAME");
	iframe.style.width = "100%", iframe.style.height = "100%", iframe.style.borderWidth = "0";
	var doc, outline = getOutline();
	container.appendChild(iframe), iframe.onload = function() {
		...
		}, !1), window.addEventListener("resize", updateHeight)
